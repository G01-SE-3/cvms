...
class DynamicSeizureSections extends StatefulWidget {
  final Function(List<Seizure>)
      onSeizuresUpdated; // Callback to send updated list
  final List<Seizure>? initialSeizures; // Optional initial list of seizures

  const DynamicSeizureSections({
    super.key,
    required this.onSeizuresUpdated,
    this.initialSeizures,
  });
...
class _DynamicSeizureSectionsState extends State<DynamicSeizureSections> {
  ...
  void initState() {
    super.initState();
    if (widget.initialSeizures != null) {
      seizures = widget.initialSeizures!;
      for (var seizure in seizures) {
        _seizureSections.add(SeizureSection(
          onSeizureUpdated: _handleSeizureUpdated,
          initialSeizure: seizure,
        ));
      }
    } else {
      _seizureSections
      ...
      seizures.add(
          Seizure(seizureAmount: '', seizureQuantity: '', seizedGoods: ''));
    }
  }
          ...
          child: TextButton.icon(
            onPressed: () {
              setState(() {
                ...
                widget.onSeizuresUpdated(
                    seizures); // Notify parent widget on adding a new section
              });
            },
            icon: const Icon(
              Icons.add,
