...
String? validateDateRange(DateTime? startDate, DateTime? endDate) {
  ...
  if (startDate != null && endDate != null && startDate.isAfter(endDate)) {
    return ValidationStrings
        .startDateError; // Return error message if invalid range
  } else if (startDate == null && endDate == null) {
    return ValidationStrings
        .emptyFieldsError; // Return error message if both dates are missing
  }
...
String? validateInput(String input) {
  if (input.isEmpty) {
    return ValidationStrings
        .emptyFieldError; // Return error message if input is empty
  } else if (double.tryParse(input) == null) {
    return ValidationStrings
        .invalidNumberError; // Return error message if input is not a valid number
  }
...
String? validateSearchInput(String? input) {
  if (input == null || input.isEmpty) {
    return ValidationStrings
        .searchInputNullError; // Return error message if input is empty
  } else if (int.tryParse(input) == null) {
    return ValidationStrings
        .invalidSearchInputError; // Return error message if input is not a valid number
  }
  return null; // No error, valid input
}
...
void handleNoPVFound(BuildContext context) {
  showDialog(
    ...
    builder: (context) {
      return AlertDialog(
        title: Text(ValidationStrings
            .noPVFoundTitle), // Error title from validation strings
        content: Text(ValidationStrings
            .noPVFoundMessage), // Error message from validation strings
        actions: [
          TextButton(
            onPressed: () {
  ...
  ScaffoldMessenger.of(context).showSnackBar(
    SnackBar(
      ...
      behavior: SnackBarBehavior
          .floating, // Makes the SnackBar appear in a floating manner
    ),
  );
}
